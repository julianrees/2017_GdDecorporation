...
#   scale_y_continuous(name = expression(''^153*Gd~'Content (% RD)'), limits = c(0,60), expand = c(0, 0)) +
#   theme(legend.title = element_blank()) + 
#   xlab(NULL) + 
...
# ggplot(orgreten[ which(orgreten$variable != "Skeleton" & orgreten$variable != "Liver" & orgreten$variable != "Soft"), ],
#        aes(x = Treatment, weight = Ave, ymin = Ave-Error, ymax = Ave+Error, fill = variable, group = variable)) + 
#   geom_col(aes(y = Ave, fill = variable), color = 'black', position = pos) +
#   geom_errorbar(position = pos) + 
...
#   scale_y_continuous(name = expression(''^153*Gd~'Content (% RD)'), limits = c(0,0.5)) +
#   theme(legend.title = element_blank()) + 
#   xlab(NULL) + 
#   ggsave(filename = '../../../Manuscripts/2017_GdDecorporation/Overleaf/biodistribution2.pdf', 
...
#   scale_fill_grey(end = 1) +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
#   scale_y_continuous(name = expression(''^153*Gd~'Content (% RD)'), limits = c(0,10.5), breaks = c(0,2,4,6,8,10)) +
#   theme(legend.title = element_blank()) + 
#   xlab(NULL) #+ 
#   #ggsave(filename = '../../../Manuscripts/2017_GdDecorporation/Overleaf/biodistribution.pdf', 
#    #      width = 5, height = 4, units = "in")
...
#allexcreta$Treatment <- factor(allexcreta$Treatment, 
#                           levels = levels(allexcreta$Treatment)[c(1:3,7,4,8,5,6)])
# str(allexcreta)
...
#   #ylab("% Activity Excreted") +
#   geom_point(aes(color = Treatment#, shape = Ligand
#                  ), size = 1.7) +
...
ddply()
ddply(mreten[which(mreten$Location == "Retained"), ], .(Group), summarize, Ave=mean(Value), Error=sd(Value))
